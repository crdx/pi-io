...
async function generateModels() {
	...
	// models.dev: Anthropic, OpenAI, Groq, Cerebras
	// OpenRouter: xAI and other OpenAI-compatible providers
	...
	// Only emit providers and APIs this fork supports.
	...
	for (const model of allModels) {
		if (!ALLOWED_PROVIDERS.has(model.provider)) continue;
		if (!ALLOWED_APIS.has(model.api)) continue;
...
`;
	...
	// Print statistics (count emitted models, not the unfiltered allModels)
	const emittedModels = Object.values(providers).flatMap((models) => Object.values(models));
	const totalModels = emittedModels.length;
	const reasoningModels = emittedModels.filter(m => m.reasoning).length;
