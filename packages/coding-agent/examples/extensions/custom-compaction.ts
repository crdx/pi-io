 ...
 *
 * This example also demonstrates using a different model (Haiku) for summarization,
...
export default function (pi: ExtensionAPI) {
	pi.on("session_before_compact", async (event, ctx) => {
		...
		// Use Haiku for summarization (cheaper/faster than most conversation models)
		const model = ctx.modelRegistry.find("anthropic", "claude-haiku-4-5");
		if (!model) {
			ctx.ui.notify(`Could not find Haiku model, using default compaction`, "warning");
			return;
		}
