...
import { estimateTextTokens } from "../../core/context-tokens.js";
import type {
	ExtensionContext,
	ExtensionRunner,
	ExtensionUIContext,
...
import { FooterComponent, formatUnitTokens } from "./components/footer.js";
...
export class InteractiveMode {
	...
	async init(): Promise<void> {
		...
		// fd powers @file autocomplete; rg backs the grep tool. Both must be on PATH.
		this.fdPath = findBinary("fd");
	...
	private formatPathWithSource(p: string, sourceInfo?: SourceInfo): string {
		if (sourceInfo) {
			const shortPath = this.formatDisplayPath(p);
	...
	private showLoadedResources(options?: { extensions?: Array<{ path: string; sourceInfo?: SourceInfo }> }): void {
		...
		if (contextFiles.length > 0) {
			const header = theme.fg("muted", resourceSummary ? `[Context] ${resourceSummary}` : "[Context]");
			const contextList = contextFiles
				.map((file) => {
					const tokens = formatUnitTokens(estimateTextTokens(file.content));
					return theme.fg("dim", `  ${this.formatDisplayPath(file.path)} (${tokens})`);
				})
				.join("\n");
			addSection(`${header}\n${contextList}`);
	...
	private async shutdown(): Promise<void> {
		...
		const resumeCommand = formatResumeCommand(this.session.sessionManager);
		if (resumeCommand) {
